...
filtered AS (
    ...
    AND overround <= 2.20  -- Relaxed: more races qualify
    AND disagreement >= 2.20  -- Lowered from 2.50: works with proxy model
    AND edge_pp >= 0.06  -- Lowered from 0.08: more conservative but captures value
    AND ev_adjusted >= 0.03  -- Lowered from 0.05: still positive EV
),
best_per_race AS (
...
filtered_b AS (
    ...
    WHERE edge_pp >= edge_min_required * 0.85  -- Relaxed: 85% of required edge
    AND ev >= 0.015  -- Lowered from 0.02: still positive
    AND overround <= 2.20  -- Relaxed: more races qualify
),
best_per_race_b AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY race_id ORDER BY edge_pp DESC) as rank_in_race
