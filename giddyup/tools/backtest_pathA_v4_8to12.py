...
print("=" * 80)
print("🏇 PATH A V4: 8-to-12 SPECIALIST")
print("=" * 80)

# ===== Configuration =====
# V4 "8to12 Specialist": Focus ONLY on profitable range
# - 8-12 odds shows +709% ROI across all tests
# - Lower edge threshold to get more volume
# - Minimal blending (model is strong here)
# - Target: 50-200 bets/year

ODDS_MIN = 7.5
ODDS_MAX = 12.5
COMMISSION = 0.02
KELLY_FRACTION = 0.10  # 1/10 Kelly (more conservative)
MAX_STAKE = 0.3  # Lower cap
EV_MIN = 0.00
FIELD_SIZE_PENALTY = 0.0

# Single edge threshold (optimized for this range)
EDGE_MIN_FLAT = 0.025  # 2.5pp (low threshold since model works here)

# Minimal blending (trust model in this range)
BLEND_LAMBDA_FLAT = 0.15  # 85% model, 15% market

print(f"\n⚙️  V4 Configuration:")
print(f"   ODDS RANGE: {ODDS_MIN:.1f} - {ODDS_MAX:.1f} (8-12 sweet spot)")
print(f"   EDGE: ≥ {EDGE_MIN_FLAT*100:.1f}pp (low threshold)")
print(f"   BLEND: λ={BLEND_LAMBDA_FLAT:.2f} (minimal market influence)")
print(f"   KELLY: 1/10 (conservative)")
print(f"   Target: 50-200 bets/year")
...
def apply_blending_v4(row):
    """V4: Use flat lambda (model is strong in this range)"""
    p = row["p_cal_band"]
    q = row["q_vigfree"]
    lam = BLEND_LAMBDA_FLAT
    return blend_to_market(p, q, lam)

test_pd["p_blend"] = test_pd.apply(apply_blending_v4, axis=1)
...
# Apply V4 flat edge threshold (simpler)
test_pd = test_valid.to_pandas()

test_pd["passes_edge"] = test_pd["edge_prob"] >= EDGE_MIN_FLAT
